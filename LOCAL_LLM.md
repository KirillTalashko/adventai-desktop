...
Промпт-шаблон под кейс — `OPT_TASK_PROMPT` (жёсткий формат «нумерованный список без воды») против общего
`LOCAL_LLM_SYSTEM`. В панели «Локальная LLM» — блок «День 29»: задача + параметры + выбор кванта → «До» (общий

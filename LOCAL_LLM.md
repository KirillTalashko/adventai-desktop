...
Режим **сравнения** двух ответов (облако vs локаль) с маркировкой; **MCP-tool-loop** для локальной модели
(Ollama умеет `tools` в `/api/chat`); при желании — стриминг токенов.

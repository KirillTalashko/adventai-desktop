...
Промпт-шаблон под кейс — `OPT_TASK_PROMPT` (продуктовый: роль консультанта + анти-галлюцинация + строгий
формат «нумерованный список 4–6 пунктов с образцом») против общего `LOCAL_LLM_SYSTEM`. В панели «Локальная LLM» — блок «День 29»: задача + параметры + выбор кванта → «До» (общий

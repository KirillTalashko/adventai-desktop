...
`src/main/resources/knowledge/` — на старте Дня 21 **13** markdown-документов (~20 страниц, ≈37 тыс. символов) + образец PDF (к Дню 24 корпус расширен до **~50** документов):
...
**Замер на старте (14 документов, День 21):** `fixed` → 34 чанка, ср. 1219 симв., **0 разделов**; `structural` → 102 чанка,
...
## Дальше (Дни 22–25) — СДЕЛАНО

Агент берёт вопрос → ищет в индексе → подмешивает найденные чанки в промпт → отвечает **со ссылкой на
источник** (Дни 22, 24–25). Плюс **reranking** (cross-encoder) для точности и метрики качества (День 23).
Всё влито в `main`; проект уже на неделе 6 (Local LLM, День 26).

...
// «нет»/«отсутствует» в самом ответе). Раньше регэксп ловил слишком широко: живые «апелляции нет»,
// «баланса нет», «справка об отсутствии судимости» из базы → ложная плашка «нет данных» на нормальном ответе.
// (?iu) — регистронезависимо + UNICODE_CASE (кириллица «Информация»↔«информаци…»); \p{L}, а не \w,
// т.к. \w в Java-регэкспе матчит только ASCII и мимо кириллицы.
 ...
 * показаны найденные (но нерелевантные) фрагменты. Матчит только мета-фразы об отсутствии ДАННЫХ в

...
В самом приложении (dev-режим → «Локальная LLM» → секция «День 30 · Обращение к сервису по HTTP») есть
HTTP-клиент к сервису: поля URL + токен и кнопки `GET /health`, `POST /chat`, «Прогнать 3 вопроса» (батч),
«Нагрузка ×6» (лимиты). URL — любой (localhost / IP в сети / домен); при 401 панель подсказывает, что токен
не совпал с `LLM_AUTH_TOKEN` сервиса. Так наглядно видно, что чат идёт **по сети** через сервис.
